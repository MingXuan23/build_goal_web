...
@section('container')
    ...
    <div class="modal fade" id="contentCardsModal" tabindex="-1" aria-labelledby="contentCardsModalLabel" aria-hidden="true">
    ...
    $(document).ready(function () {
        var table = $('.data-table').DataTable({
            ...
            columns: [
                { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false }, // Fixed ID for numbering
                { data: 'name', name: 'name' }, // Content Name
                { data: 'created_at', name: 'created_at' }, // Paid At
                { data: 'number_of_card', name: 'number_of_card' },
                { 
                    data: 'action', 
                    name: 'action', 
                    orderable: false, 
                    ...
                    render: function (data, type, row) {
                        return '<button class="btn btn-primary btn-sm view-cards" data-content-id="' + row.id + '">View Cards</button>';
                    }
                },
                { 
                    data: 'receipt', 
                    ...
                    render: function (data, type, row) {
                        ...
                        return '<button class="btn btn-primary btn-sm "> <a class="text-white" href="' + data + '" target="_blank"">View Receipt</a></button>';
                    }
        ...
        $('.data-table').on('click', '.view-cards', function () {
            ...
            $('#contentCardsModal').data('content-id', contentId); // Update contentId in modal's data attribute
    ...
    function appendCardRow(card = {}) {
        ...
        return { date, time }; // Return as object with date and time
    };
        ...
        const { date, time } = formatDatetime(card.startdate || `${lastStartDate} ${lastStartTime}`);
  ...
  $.ajaxSetup({
        ...
        $.ajax({
            ...
            error: function (xhr) {
                var errorMessage = xhr.responseJSON && xhr.responseJSON.error ? xhr.responseJSON.error : 'Error saving cards. Please try again.';
