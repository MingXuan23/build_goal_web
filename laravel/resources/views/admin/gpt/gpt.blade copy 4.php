    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function displayResponse(message) {
                ...
                // Flag untuk parsing HTML tag
                let isTag = false;
                let tagBuffer = '';

                // Flag untuk parsing bold (pakai **)
                let isBold = false;
                let boldTextBuffer = '';

                // Flag dan buffer untuk blok code
                let isCodeBlock = false;
                let codeBlockBuffer = '';
                let codeLang = ''; // misal 'sql', 'html', 'javascript', dsb
                let insideLangHeader = false; // untuk menangkap bahasa setelah ``` misalnya ```sql

                // Kosongkan container
                responseContainer.html('');

                const intervalId = setInterval(() => {
                    // Berhenti jika sudah habis
                    if (messageIndex >= message.length) {
                        clearInterval(intervalId);
                        // Setelah selesai, panggil Prism untuk highlight
                        Prism.highlightAll();
                        return;
                    }
                    ...
                    // Cek apakah kita mendapati awal triple backticks
                    // (pastikan kita punya cukup sisa karakter)
                    if (!isCodeBlock && char === '`' &&
                    messageIndex + 1 < message.length &&
                    message[messageIndex] === '`' &&
                        message[messageIndex + 1] === '`') {
                    // Masuk ke mode code block
                    isCodeBlock = true;
                    codeBlockBuffer = ''; // kosongkan buffer
                    codeLang = '';
                    insideLangHeader = true; // berikutnya kita cek bahasa di sebaris
                    // Lewatkan 2 karakter lagi (karena kita sudah tahu ini ```)
                        messageIndex += 2;
                        return;
                    }

                    // Cek apakah kita mendapati akhir triple backticks (penutup)
                    if (isCodeBlock && char === '`' &&
                    messageIndex + 1 < message.length &&
                    message[messageIndex] === '`' &&
                        message[messageIndex + 1] === '`') {
                    // Tutup code block
                    isCodeBlock = false;
                    // Lewatkan 2 karakter lagi
                    messageIndex += 2;

                    // Render buffer codeBlockBuffer jadi <pre><code>...
                    // Misal: <pre class="language-sql"><code>SELECT * FROM users;</code></pre>
                    let languageClass = codeLang ? `language-${codeLang}` : 'language-none';

                    // Kita ganti karakter HTML khusus agar < & > dsb aman
                    const escapedCode = escapeHtml(codeBlockBuffer);
                    ...
                    const codeHtml = `
                        ...
                        `;
                    formattedMessage += codeHtml;
                    responseContainer.html(formattedMessage);
                    $('#main-chat-content').scrollTop($('#main-chat-content')[0].scrollHeight);

                    return; // lanjut loop berikutnya
                }

                // Jika kita sedang di dalam code block
                if (isCodeBlock) {
                    // Cek jika kita masih di "lang header" (misal ```sql)
                        if (insideLangHeader) {
                            // Jika ketemu \n atau spasi, berarti lang header selesai
                            if (char === '\n' || char === ' ') {
                                insideLangHeader = false;
                            } else {
                                // Tambahkan ke codeLang
                                codeLang += char;
                                return; // lanjut
                            }
                        } else {
                            // Simpan semua karakter ke buffer code
                            codeBlockBuffer += char;
                        }

                        // Update tampilan partial agar “efek ketik” tampak
                        // (opsional, jika mau code block muncul seiring pengetikan)
                        // Di contoh ini, kita *tidak* menampilkan code block sampai penutup ditemukan,
                        // jadi tidak menambah ke formattedMessage di sini.
                        responseContainer.html(formattedMessage);
                        $('#main-chat-content').scrollTop($('#main-chat-content')[0].scrollHeight);
                        return;
                    }

                    // ---- Jika bukan code block, lanjut parse tag < >, bold **, dsb ----
                    ...
                    if (char === '<') {
                        ...
                        // Jika masih di dalam tag
                        tagBuffer += char;
                    } else {
                        // Deteksi double asterisk untuk toggle bold
                        if (char === '*' && message.charAt(messageIndex) === '*') {
                            ...
                            isBold = !isBold;
                            messageIndex++; // skip bintang berikut
                        } else {
                            // Jika sedang bold, tambahkan ke buffer bold
                            if (isBold) {
                                boldTextBuffer += (char === '\n') ? '<br>' : char;
                            } else {
                                // Biasa
                    ...
                    responseContainer.html(formattedMessage);
                    $('#main-chat-content').scrollTop($('#main-chat-content')[0].scrollHeight);
                }, 10); // 10 ms per char, silakan ubah sesuai kebutuhan
